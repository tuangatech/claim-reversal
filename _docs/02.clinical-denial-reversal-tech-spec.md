...
graph TD
    ...
    subgraph MainApp [":8000 — Main App"]
        API["⚡ FastAPI endpoints"]
        Orch["🔀 AppealCrew Orchestrator"]
        Tools["Direct tool calls:\nIntake · Physician · Submission\n(rules-based)"]
        Writer["CrewAI Agent:\nAppeal Writer\n(LLM reasoning loop)"]
        API --> Orch
        Orch --> Tools
        Orch --> Writer
    ...
    EvidenceAgent["Clinical Evidence Agent :8001\nLangGraph + A2A Server"]
    MCP["🔧 MCP Server :8002\nShared Tools"]
    DB[("💾 SQLite\nclaims.db")]

    UI -->|"POST /appeal\nGET /stream (SSE)"| API
    Orch -->|"A2A task lifecycle"| EvidenceAgent
    Orch -->|"MCP client (SSE)"| MCP
    EvidenceAgent -->|"MCP client (SSE)"| MCP
    MCP --> DB
    MainApp --> DB
```
