# Claim Denied: A Multi-Agent Clinical Appeal Reversal System
...
15. [Playwright Web Automation (Stretch Goal)](#15-playwright-web-automation-stretch-goal)
16. [Mock Data Strategy](#16-mock-data-strategy)
17. [Technology Decision Log](#17-technology-decision-log)
...
# Stretch goal dependencies (Phase 5 only)
uv add --optional playwright playwright pdfplumber
```
...
# Feature flags
USE_PLAYWRIGHT=false       # Set true only for stretch goal
...
**Orchestration Layer** — A CrewAI `Crew` runs a sequential pipeline of agents, each responsible for one phase of the appeal workflow. CrewAI manages task handoff, context passing, and final output assembly. The Clinical Evidence Agent participates as a remote A2A crew member — the crew runner (in `appeal_crew.py`) manages the A2A task lifecycle directly, including HITL suspension and resume (see Section 9.5).
...
**Shared Tool Layer** — A lightweight MCP server exposes shared tools that multiple agents consume: clinical guideline lookup, payer appeal rules, audit event logging, and claim history. Agents connect to the MCP server via the SSE transport.
...
graph TD
    ...
    API["⚡ FastAPI\nMain App"]
    CREW["🤝 CrewAI Crew\nOrchestrator"]
    INTAKE["Agent: Denial Intake"]
    EVIDENCE["Agent: Clinical Evidence\n(LangGraph + A2A Server)"]
    WRITER["Agent: Appeal Writer"]
    PHYSICIAN["Agent: Physician Sign-off"]
    ...
    INTAKE & WRITER & PHYSICIAN & SUBMISSION -->|"MCP tools"| MCP
    API --> DB
    MCP --> DB
```
...
graph LR
    ...
    subgraph Main Process [:8000]
        ...
        CREW_RUNNER["CrewAI Crew Runner"]
        API --> SSE
        API --> CREW_RUNNER
    end

    subgraph CrewAI Crew
        INTAKE_A["Denial Intake Agent\nCrewAI + local tools"]
        WRITER_A["Appeal Writer Agent\nCrewAI + local tools"]
        PHYSICIAN_A["Physician Sign-off Agent\nCrewAI + local tools"]
        SUBMISSION_A["Submission Agent\nCrewAI + local tools"]
        INTAKE_A --> WRITER_A --> PHYSICIAN_A --> SUBMISSION_A
    end
    ...
    UI -->|HTTP + SSE| API
    CREW_RUNNER --> INTAKE_A
    CREW_RUNNER -->|"A2A tasks/send\n(task lifecycle)"| A2A_ENDPOINT
    INTAKE_A & WRITER_A & PHYSICIAN_A & SUBMISSION_A -->|"mcps=[]"| MCP_SERVER
    MCP_SERVER --> DB
    API --> DB
```
...
|---|---|---|---|---|
| `main-app` | FastAPI app, CrewAI Crew, all local agent tools, SSE emitter | 8000 | Python / uvicorn | Entry point for all UI requests |
...
| # | Agent Name | Framework | Local / Remote | Responsibility |
|---|---|---|---|---|
| 1 | Denial Intake Agent | CrewAI | Local | Triages the denial, classifies type, checks deadline, scores worthiness |
...
| 3 | Appeal Writer Agent | CrewAI | Local | Assembles the formal appeal letter from denial context + evidence bundle |
| 4 | Physician Sign-off Agent | CrewAI | Local | Routes appeal for physician attestation if required; simulates approval |
| 5 | Submission Agent | CrewAI | Local | Submits the appeal to the payer and records the confirmation; workflow terminal point |

---

## 7. Agent Specifications

### 7.1 Denial Intake Agent

**Responsibility**: Entry point for the workflow. Receives denial data, classifies the denial type, validates the appeal window, and scores the claim for appeal worthiness. Outputs a `DenialRecord` with a routing decision. A2A delegation to the Evidence Agent is handled by the crew runner (see Section 9.5), not by this agent.

**Framework**: CrewAI Agent

**Tools**:
- `triage_denial` (local) — accepts structured denial input, returns classification (clinical/admin), deadline status, and worthiness score in one call. Uses prior appeal history from `get_claim_history` to penalize the worthiness score (e.g., `score -= 15` if a prior appeal for the same reason was already denied).
- `get_claim_history` (MCP) — retrieves prior denials and payments for the same claim. Result feeds into `triage_denial` worthiness scoring.
- `log_appeal_event` (MCP) — writes intake event to audit log

**Inputs**: Structured denial JSON (claim ID, denial reason code, claim amount, payer ID, date of service)
**Outputs**: Structured `DenialRecord` + routing decision (proceed / close / skip)
**Decision logic**: If denial type ≠ clinical → exit workflow. If deadline passed → close. If worthiness score < threshold (accounting for prior appeal penalty) → close. Otherwise → proceed (crew runner handles A2A delegation next).
...
- `lookup_clinical_guideline` (MCP) — returns InterQual/MCG criteria for a diagnosis code
- `evaluate_evidence_sufficiency` (local) — LLM-scored check: does the gathered evidence meet the payer's criteria?
- `fetch_lcd_policy` (local, Playwright — stretch goal) — downloads a CMS LCD policy PDF and extracts criteria text
- `log_appeal_event` (MCP) — writes evidence events to audit log
...
**Responsibility**: Assembles the formal appeal letter using the denial context and the evidence bundle. Formats the letter to match the payer's requirements. Returns a ready-to-submit document.

**Framework**: CrewAI Agent

**Tools**:
- `get_payer_appeal_rules` (MCP) — returns payer-specific submission format, required sections, and deadline rules
- `draft_appeal_letter` (local) — LLM-powered tool that composes and formats the letter from structured inputs
- `log_appeal_event` (MCP) — writes drafting event to audit log

**Inputs**: `DenialRecord` + `EvidenceBundle`
**Outputs**: `AppealLetter` (formatted text, payer submission metadata)
**Decision logic**: None — linear execution.

---

### 7.4 Physician Sign-off Agent

**Responsibility**: Determines if physician attestation is required for this appeal. If yes, simulates physician review and approval (always approves). If not required, passes through immediately. Kept as a distinct agent to maintain 1:1 mapping between crew tasks and real-world workflow stages (visible in the SSE timeline).
...
**Outputs**: `SignedAppealLetter` or pass-through
**Decision logic**: If physician required → simulate approval, log event, mark letter as physician-attested. If not required → pass through unchanged. No branching, no failure path.

---

### 7.5 Submission Agent

**Responsibility**: Submits the signed appeal to the payer through the required channel. Records the submission confirmation. This is the terminal step.

**Framework**: CrewAI Agent

**Tools**:
- `submit_appeal` (local) — simulates submission to payer portal; returns a confirmation number and timestamp
- `log_appeal_event` (MCP) — writes submission event to audit log

**Inputs**: `SignedAppealLetter` + `DenialRecord`
**Outputs**: `SubmissionConfirmation` (confirmation number, submitted_at, payer_id, method)
**Decision logic**: None — linear execution. Always submits if it receives a signed letter.
...
class AppealState(TypedDict):
    ...
    lcd_policy_text: Optional[str]           # stretch goal — empty unless Playwright enabled
...
| `lookup_guideline` | Calls MCP `lookup_clinical_guideline` with diagnosis codes; populates `guideline_citations` |
| `fetch_lcd` | Calls `fetch_lcd_policy` (mock returns empty string; Playwright when enabled); populates `lcd_policy_text` |
...
**Mechanism**: LangGraph's `interrupt()` (from `langgraph.types`) suspends the graph and returns control to the caller. The A2A server catches the suspension, returns `status: "input-required"` to the crew runner, and holds state in a `MemorySaver` checkpointer (from `langgraph.checkpoint.memory`). The crew runner then pushes the `human_review_required` event to the SSE queue (see Section 14.3 for SSE emission architecture).
...
The Clinical Evidence Agent runs as a standalone FastAPI server that exposes an A2A-compliant interface. The crew runner (in `appeal_crew.py`) manages A2A communication directly — sending tasks, handling the `input-required` HITL signal, and resuming after human input. This simulates a real organizational boundary — the evidence service could be maintained by a different team or run in a different environment.

A2A delegation is a **crew-level concern**, not an agent-level concern. The Intake Agent triages and outputs a `DenialRecord`; the crew runner then sends it to the Evidence Agent via A2A as the next step in the pipeline.
...
### 9.3 A2A Client Configuration (crew runner)

The crew runner connects to the remote evidence agent using `httpx.AsyncClient` with the A2A protocol's `tasks/send` endpoint. Configuration:
...
| `fail_fast` | `False` | If the remote agent is unreachable, crew runner emits SSE error and terminates gracefully — no crash |
...
sequenceDiagram
    ...
    participant CREW as Crew Runner
    participant INTAKE as Denial Intake Agent
    ...
    Note over CREW: Crew runner owns A2A lifecycle from here
    ...
    alt evidence sufficient
        ...
        Note over CREW: Crew runner stores task_id
    ...
    Note over CREW: Crew runner feeds EvidenceBundle to Writer task
    CREW->>CREW: continue to Writer → Physician → Submission
```

### 9.5 HITL Handoff via A2A Task Lifecycle

When the LangGraph `interrupt()` fires inside the Evidence Agent, the A2A server must signal this back to the crew runner without blocking an HTTP connection for minutes. The A2A protocol's task lifecycle handles this natively.

**Sequence:**

1. Crew runner sends initial `tasks/send` to A2A server with the `DenialRecord` payload. A new `task_id` is generated server-side.
...
4. **Crew runner stores the `task_id`** from this response. This is critical — it's the correlation key for resuming the suspended graph.
5. Crew runner emits SSE `human_review_required` to the browser and awaits on a per-claim `asyncio.Event`.
6. Human POSTs to `POST /appeal/{claim_id}/resume` with `{ choice: "proceed" | "close" }`. The main app sets the event.
7. Crew runner wakes, sends a **second `tasks/send`** to the A2A server with:
...
10. Crew runner continues to the Writer task (or terminates the workflow if closed).

**Key implementation details:**

| Concern | Resolution |
|---|---|
| `task_id` storage | Crew runner holds `task_id` in a local variable between steps 4 and 7. Not persisted — if the main app crashes, the workflow is lost (acceptable for a learning project). |
...
| Timeout ownership | The **main app** owns the timeout. If `HITL_TIMEOUT_SECONDS` elapses without a human response, the crew runner sends the second `tasks/send` with `choice: "close"` and `reason: "timeout"`. The A2A server doesn't track time. |
| Connection failure on resume | If the second `tasks/send` fails (A2A server restarted), the `MemorySaver` state is lost (in-memory only). The crew runner emits SSE `error` and terminates. Acceptable for local dev. |
...
The crew runner sends a task object with the **same `id`** (the `task_id` stored from the first response). The `messages` array contains a single user message with one `data` part. The data part carries a single field: `human_choice` set to either `"proceed"` or `"close"`.
...
graph LR
    ...
    INTAKE["Denial Intake Agent"] -->|"get_claim_history\nlog_appeal_event"| MCP
    EVIDENCE["Clinical Evidence Agent"] -->|"lookup_clinical_guideline\nlog_appeal_event"| MCP
    WRITER["Appeal Writer Agent"] -->|"get_payer_appeal_rules\nlog_appeal_event"| MCP
    SUBMISSION["Submission Agent"] -->|"log_appeal_event"| MCP

    T3 & T4 --> DB[("claims.db")]
```

### 10.2 Agent Connection

Each CrewAI agent connects to the MCP server via the `mcps` field with the SSE URL: `mcps=["http://localhost:8002/sse"]`. CrewAI also supports structured `MCPServerSSE(url=...)` objects for more control (headers, tool filtering), but the string DSL is sufficient here.

The Clinical Evidence Agent (running in the A2A server process) connects identically using the same MCP URL.
...
|---|---|---|---|---|
| `triage_denial` | Intake | Structured denial JSON (claim_id, reason_code, amount, payer_id, dos) | `{type, is_clinical, days_remaining, is_open, worthiness_score, recommendation}` | Rules-based: code→type map, date arithmetic, amount threshold |
...
| `draft_appeal_letter` | Writer | DenialRecord + EvidenceBundle + payer rules | Formatted `AppealLetter` text | LLM composition via agent model |
| `physician_review` | Physician | AppealLetter, DenialRecord | `{required: bool, status: "approved"}` | Rules-based; always approves (simulated) |
| `submit_appeal` | Submission | AppealLetter, payer_id | `{confirmation_number, submitted_at, method}` | Mock: generates confirmation, logs to DB |
...
### 11.3 Stretch Goal Tool (1)

| Tool | Owner Agent | Input | Output | Mock Strategy |
|---|---|---|---|---|
| `fetch_lcd_policy` | Evidence (A2A) | diagnosis_code | `{policy_text, source_url}` | Mock: returns empty string; Stretch: Playwright → CMS |

**Total: 10 tools (6 local + 4 MCP) + 1 stretch**
...
stateDiagram-v2
    ...
    LookingUpGuidelines --> FetchingLCD : fetch_lcd node
    FetchingLCD --> Evaluating : evaluate_sufficiency node
...
Valid `step` values: `intake_complete`, `evidence_gathering`, `evidence_gathered`, `evidence_human_review`, `letter_drafted`, `physician_approved`, `appeal_submitted`, `case_closed`
...
│
├── crew/
│   └── appeal_crew.py         # Crew definition, task wiring, SSE hooks
│
...
│       └── fetch_lcd_policy.py    # Mock first; Playwright later (stretch)
│
...
│       └── claim_history.py
│
├── tools/                     # Local tools for CrewAI agents
...
│   └── llm.py                # OpenRouter helper (get_agent_llm)
│
├── data/
...
│   └── downloads/             # Playwright PDF downloads (stretch goal)
│
├── frontend/
...
graph TD
    subgraph "index.html"
        HEADER["Header: Claim Denied — Appeal Workflow"]
...
**SSE emission architecture** — centralized at the crew level, not inside individual agents:
- **Primary emitter**: CrewAI's `step_callback` hook on the Crew. Fires after each task completes. The callback pushes a `workflow_step` event to the per-claim `asyncio.Queue`. Individual agents never touch the queue directly — they are decoupled from web infrastructure.
- **A2A HITL exception**: When the crew runner receives `status: "input-required"` from the A2A evidence server, it pushes `human_review_required` to the queue directly. This is the only SSE event not triggered by `step_callback`.
- **Separation from audit log**: `log_appeal_event` (MCP tool) writes to SQLite for the audit trail. SSE events are a separate concern — they drive the UI timeline. An agent calling `log_appeal_event` does NOT also emit an SSE event; the `step_callback` handles UI notification independently.

### 14.4 Demo Script

1. Start all three processes (`main-app`, `evidence-agent`, `mcp-server`)
2. Open `index.html` in a browser
3. Enter a pre-seeded claim ID with **insufficient evidence** (e.g., `CLM-2024-00199`) and click Submit
...
8. Resubmit with a **sufficient evidence** claim ID (e.g., `CLM-2024-00123`) — watch the full happy path with no HITL pause
...
## 15. Playwright Web Automation (Stretch Goal)

### 15.1 Purpose

The `fetch_lcd_policy` tool navigates the CMS Medicare Coverage Database to find the Local Coverage Determination (LCD) document for a given diagnosis code, downloads the policy PDF, and extracts relevant medical necessity criteria text. This simulates what a billing specialist does manually when building an appeal.

### 15.2 Target Workflow

```mermaid
flowchart LR
    START(["fetch_lcd_policy(diagnosis_code)"])
    LAUNCH["Launch headless\nChromium via Playwright"]
    NAV["Navigate to\nlcd.cms.gov"]
    SEARCH["Enter diagnosis code\nin search field"]
    FILTER["Filter results by\nMAC jurisdiction"]
    SELECT["Click first active LCD\nresult"]
    DOWNLOAD["Locate PDF attachment\nand download"]
    EXTRACT["Extract text from PDF\nusing pdfplumber"]
    RETURN(["Return {policy_text, source_url}"])

    START --> LAUNCH --> NAV --> SEARCH --> FILTER --> SELECT --> DOWNLOAD --> EXTRACT --> RETURN
```

### 15.3 Mock-First Implementation Path

| Phase | Implementation | Graph behavior |
|---|---|---|
| Phases 1–5 | `fetch_lcd_policy` returns empty string (no-op) | Graph runs identically; `lcd_policy_text` is optional |
| Stretch | Playwright implementation replaces internals | Graph unchanged — only the tool internals change |

Controlled by `USE_PLAYWRIGHT` env var. When `false`, the tool is a no-op returning `{"policy_text": "", "source_url": ""}`.

### 15.4 Dependencies

```toml
[project.optional-dependencies]
playwright = ["playwright>=1.40", "pdfplumber>=0.10"]
```
...
| Data | Variant A (CLM-2024-00123) | Variant B (CLM-2024-00199) |
|---|---|---|
...
data/
├── mock_eobs/
│   ├── CLM-2024-00123.json    # Maria Santos pneumonia denial
│   └── CLM-2024-00199.json    # James Chen knee replacement denial
├── mock_records/
│   ├── CLM-2024-00123.json    # Pneumonia clinical records (sufficient)
│   └── CLM-2024-00199.json    # Knee replacement records (insufficient)
```
...
## 17. Technology Decision Log

| Decision | Chosen | Alternatives Considered | Rationale |
|---|---|---|---|
| **Top-level orchestration** | CrewAI | Pure LangGraph, AutoGen | CrewAI's Crew + Task model maps naturally to the sequential appeal phases; first-class A2A support |
...
|---|---|---|
| 1 | Each CrewAI agent has a clear, bounded responsibility with no overlap | Code review: each agent's tool list covers exactly its workflow phase |
| 2 | LangGraph graph routes to `sufficient` on first try (Variant A) | Run with CLM-2024-00123; assert `state["sufficiency"] == SUFFICIENT` and `retry_count == 0` |
| 3 | LangGraph retry loop works: insufficient → retry → still insufficient (Variant B) | Run with CLM-2024-00199; assert `retry_count >= 2` |
| 4 | LangGraph `interrupt()` triggers after 2 failed retries | Run with CLM-2024-00199; assert graph suspends and `hitl_triggered == True` |
...
| 14 | Playwright `fetch_lcd_policy` works (stretch) | Run standalone; assert returned text contains policy language and valid CMS URL |
...
- `data/mock_eobs/CLM-2024-00123.json` and `CLM-2024-00199.json` exist with realistic content
...
- `triage_denial` with CLM-2024-00123 returns `{is_clinical: true, is_open: true, recommendation: "proceed"}`
...
**What to build**: `a2a_server/state.py`, `a2a_server/graph.py` with all nodes (including `fetch_lcd` as no-op), conditional edges, `interrupt()` HITL, `MemorySaver` checkpointer. A2A server on port 8001 with agent card. `POST /appeal/{claim_id}/resume` endpoint.

**Verify**:
- Variant A (CLM-2024-00123): graph completes with `sufficiency == "sufficient"`, `retry_count == 0`
- Variant B (CLM-2024-00199): graph suspends at `interrupt()`, resume with `proceed` → partial bundle, resume with `close` → `closed_reason` set
...
**What to build**: All 4 CrewAI agents in `agents/`, crew definition in `crew/appeal_crew.py` with A2A delegation to evidence agent. FastAPI endpoints (`POST /appeal`, `GET /stream/{id}`, `POST /appeal/{id}/resume`). SSE event queue. `index.html` with EventSource, timeline UI, HITL decision card. HITL timeout logic.

**Verify**:
- Submit CLM-2024-00123 via browser: full happy path, no HITL, appeal letter rendered
- Submit CLM-2024-00199 via browser: HITL card appears, click Proceed → partial letter, click Close → case closed
- HITL timeout: set 5s, submit CLM-2024-00199, don't click → auto-closes
...
**What to build**: Error handling, edge cases, demo polish. Optionally: Playwright `fetch_lcd_policy` implementation (controlled by `USE_PLAYWRIGHT` env var).

**Verify**:
- All 14 success criteria pass
- Demo script (Section 14.4) runs smoothly end-to-end
- (Stretch) `fetch_lcd_policy("J18.9")` with Playwright returns real CMS policy text
