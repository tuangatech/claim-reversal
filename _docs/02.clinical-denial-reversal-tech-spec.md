...
Valid `step` values: `input_validating`, `input_validated`, `input_rejected`, `intake_complete`, `evidence_gathering`, `evidence_gathered`, `evidence_human_review`, `letter_validating`, `letter_validated`, `letter_drafted`, `physician_approved`, `appeal_submitted`, `case_closed`
...
| **Letter validation** | Writer self-validates via tool call | Separate validator agent, no validation | Keeps the Writer's scope intact; one retry on failure avoids infinite loops; no new agent overhead |
